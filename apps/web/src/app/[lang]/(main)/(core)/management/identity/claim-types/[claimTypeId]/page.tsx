...
export default async function Page({
  ...
  params: { lang: string; claimTypeId: string };
}) {
  ...
  const claimTypeDetailsResponse =
    await getClaimTypeDetailsByIdApi(claimTypeId);
  if (isErrorOnRequest(claimTypeDetailsResponse, lang, false)) {
    return (
      <ErrorComponent
        languageData={languageData}
        message={claimTypeDetailsResponse.message}
      />
    );
  }
  ...
  return (
    <>
      <Form
        languageData={languageData}
        response={claimTypeDetailsResponse.data}
      />
